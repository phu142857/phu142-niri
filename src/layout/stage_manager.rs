...
impl<W: LayoutElement> StageManagerState<W> {
    ...
    pub fn on_window_activated(&mut self, id: &W::Id, max_cast: usize) -> bool {
        if self.is_stage_window(id) {
            return false;
        }
        ...
        self.promote_cast_to_stage(id, max_cast)
    }
    ...
    pub fn promote_cast_to_stage(&mut self, id: &W::Id, max_cast: usize) -> bool {
        if self.is_stage_window(id) {
            return false;
        }
        ...
        self.set_stage_single(target_id, max_cast);
        true
    }
    ...
    /// Drag a stage window to the cast strip: split into its own cast group.
    pub fn on_window_dragged_to_cast(&mut self, id: W::Id, max_cast: usize) -> bool {
        if !self.is_stage_window(&id) {
            return false;
        }
        ...
        if let Some(active) = &mut self.active_group {
            if active.windows.len() == 1 {
                split = self.active_group.take();
            } else {
                active.remove(&id);
                split = Some(StageGroup::new(id));
            }
        }

        let Some(group) = split else {
            return false;
        };
    ...
    fn set_stage_single(&mut self, id: W::Id, max_cast: usize) {
        ...
        Self::remove_from_group_list(&mut self.cast_groups, &id);
        Self::remove_from_group_list(&mut self.hidden_groups, &id);
        self.demote_active_to_cast();
        // The window may have landed in cast when the previous active group was demoted.
        Self::remove_from_group_list(&mut self.cast_groups, &id);
        Self::remove_from_group_list(&mut self.hidden_groups, &id);
    ...
    fn demote_active_to_cast(&mut self) {
...
pub fn strip_drag_end<W: LayoutElement>(
    ...
    {
        workspace.stage_manager_save_active_sizes();
        state.on_window_dragged_to_cast(window.clone(), config.max_cast_groups)
    } else {
        false
    };
